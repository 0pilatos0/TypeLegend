 ...
 */
export function partition<T>(arr: T[], predicate: (element: T, index: number, array: T[]) => boolean): [T[], T[]] {
  if (arr.length === 0 || arr.every((element, index, array) => !predicate(element, index, array))) {
    return [[], []];
  }
